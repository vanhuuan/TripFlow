...
export function TripStepForm({ step, submitLabel, isSaving, serverError, currencyCode = "USD", onSubmit }: TripStepFormProps) {
  const [files, setFiles] = useState<File[]>([]);
  const [uploadError, setUploadError] = useState<string | null>(null);
  const [isUploading, setIsUploading] = useState(false);
  const { register, handleSubmit, setError, formState: { errors } } = useForm<StepFormValues>({ defaultValues: { title: step?.title ?? "", description: step?.description ?? "", type: step?.type ?? "Place", scheduledAt: step?.scheduledAt ? step.scheduledAt.slice(0, 16) : "", costAmount: step?.costAmount ?? "", googleMapsUrl: step?.googleMapsUrl ?? "", externalUrl: step?.externalUrl ?? "" } });
