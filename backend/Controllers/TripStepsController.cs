...
public class TripStepsController(AppDbContext dbContext, ICurrentUserService currentUserService) : ControllerBase
{
    ...
    public async Task<ActionResult<TripStepResponse>> Create(Guid tripId, CreateTripStepRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!ValidateStepRequest(normalizedRequest.Title, normalizedRequest.Type)) return ValidationProblem(ModelState);
    ...
    private bool ValidateStepRequest(string title, TripStepType type) { if (string.IsNullOrWhiteSpace(title)) ModelState.AddModelError(nameof(CreateTripStepRequest.Title), "Title is required."); if (!Enum.IsDefined(type)) ModelState.AddModelError(nameof(CreateTripStepRequest.Type), "Step type is invalid."); return ModelState.IsValid; }
    ...
    private static TripStepResponse ToResponse(TripStep step) => new(step.Id, step.TripId, step.Title, step.Description, step.Type, step.Status, step.ScheduledAt, step.GoogleMapsUrl, step.ExternalUrl, DeserializeImageUrls(step.ImageUrlsJson), step.OrderIndex, step.CreatedAt, step.UpdatedAt);
    private sealed record NormalizedStepRequest(string Title, string? Description, TripStepType Type, DateTimeOffset? ScheduledAt, string? GoogleMapsUrl, string? ExternalUrl, IReadOnlyList<string> ImageUrls);
}
