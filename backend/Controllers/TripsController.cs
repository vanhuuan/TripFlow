...
{
    [HttpPost]
    public async Task<ActionResult<TripDetailResponse>> Create(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!ValidateTripRequest(normalizedRequest.Title, normalizedRequest.Destination, normalizedRequest.StartDate, normalizedRequest.EndDate))
        {
    ...
    public async Task<ActionResult<IReadOnlyList<TripSummaryResponse>>> GetAll(CancellationToken cancellationToken)
    {
        ...
        var trips = await dbContext.Trips
            ...
            .Select(trip => ToSummaryResponse(trip))
    ...
    [HttpPut("{tripId:guid}")]
    public async Task<ActionResult<TripDetailResponse>> Update(
        Guid tripId,
    ...
    public async Task<ActionResult<TripDetailResponse>> Start(Guid tripId, CancellationToken cancellationToken)
    {
        return await UpdateStatus(tripId, TripStatus.Active, cancellationToken);
    }

    [HttpPost("{tripId:guid}/complete")]
    public async Task<ActionResult<TripDetailResponse>> Complete(Guid tripId, CancellationToken cancellationToken)
    {
        return await UpdateStatus(tripId, TripStatus.Completed, cancellationToken);
    }

    private async Task<ActionResult<TripDetailResponse>> UpdateStatus(
        Guid tripId,
        TripStatus status,
        CancellationToken cancellationToken)
    {
    ...
    private async Task<Trip?> GetOwnedTrip(Guid tripId, Guid userId, CancellationToken cancellationToken)
    {
    ...
    private bool ValidateTripRequest(string title, string destination, DateOnly? startDate, DateOnly? endDate)
    {
        ...
        {
            ModelState.AddModelError(nameof(CreateTripRequest.Title), "Title is required.");
        }

        if (string.IsNullOrWhiteSpace(destination))
        {
            ModelState.AddModelError(nameof(CreateTripRequest.Destination), "Destination is required.");
        }

        if (startDate.HasValue && endDate.HasValue && endDate.Value < startDate.Value)
        {
            ModelState.AddModelError(nameof(CreateTripRequest.EndDate), "End date must be on or after start date.");
        }

        return ModelState.IsValid;
    }

    private static NormalizedTripRequest NormalizeRequest(CreateTripRequest request) =>
        new(
            request.Title.Trim(),
            ...
            request.StartDate,
            request.EndDate,
            NormalizeOptionalString(request.CoverImageUrl));

    private static NormalizedTripRequest NormalizeRequest(UpdateTripRequest request) =>
        new(
            request.Title.Trim(),
            request.Destination.Trim(),
            NormalizeOptionalString(request.Description),
    ...
    private static TripSummaryResponse ToSummaryResponse(Trip trip) =>
        new(
            trip.Id,
            trip.Title,
            trip.Destination,
    ...
    private static TripDetailResponse ToDetailResponse(Trip trip) =>
        new(
            trip.Id,
            trip.Title,
            trip.Destination,
    ...
    private static TripStepResponse ToStepResponse(TripStep step) =>
        new(
            step.Id,
            step.TripId,
            step.Title,
    ...
    private sealed record NormalizedTripRequest(
        string Title,
        string Destination,
        string? Description,
        DateOnly? StartDate,
